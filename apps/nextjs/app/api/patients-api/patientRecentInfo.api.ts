...
export async function fetchPatientLatestReport(
  patientId: string,
  router: any // Pass router instance as a parameter
): Promise<any> {
  const patientUuid = patientId.toUpperCase();
  try {
    ...
    // Make the API request to fetch the patientLatestReport
    const response = await axios.post(
      `${apiUrl}/patient-information/latest-report/${patientUuid}`,
      {},
      { headers }
    );

    // Handle the response data
    const patientLatestReport = response;

    console.log("api patientLatestReport", patientLatestReport);
    return patientLatestReport;
  } catch (error: any) {
    if (axios.isAxiosError(error)) {
      const axiosError = error as AxiosError;
      if (axiosError.message === "Network Error") {
    ...
    console.error("Error searching patientLatestReport:", error.message);
    return Promise.reject(error);
  }
}