...
import NofileviewAppointmentsModalContent from "./nofileview-appointments-modal-content";
...
export const AppointmenViewModalContent = ({
  ...
  const handleDeleteClick = async () => {
    ...
    try {
      ...
      if (selectedFileUUID) {
        // Call the delete function with the appointment and selectedFileUUID
        await deleteAppointmentFile(appointmentUuid, selectedFileUUID);
  ...
  const [currentFile, setCurrentFile] = useState<AppointmentFile>(
    {} as AppointmentFile,
  );
  ...
  const prevFile = () => {
    if (fileIndex > 0) {
      ...
      setCurrentFile(AppointmentFiles[fileIndex - 1]);
    }
  };
  const nextFile = () => {
    if (fileIndex < AppointmentFiles.length - 1) {
      setFileIndex(fileIndex + 1);
      setCurrentFile(AppointmentFiles[fileIndex + 1]);
    }
  };
  ...
  useEffect(() => {
    if (AppointmentFiles && AppointmentFiles.length > 0) {
      const file = AppointmentFiles[fileIndex];
      setCurrentFile(file);
  ...
  }, [fileIndex, AppointmentFiles]);

  //fetching files from database
  useEffect(() => {
    setIsSubmitted(true);
    console.log(
      "fetching appointment files",
      appointmentData.appointments_uuid,
    );

    const fetchData = async () => {
      setAppointmentUuid(appointmentData.appointments_uuid);
      try {
        const response = await fetchAppointmentFiles(
          appointmentData.appointments_uuid,
          router,
        );

        if (response.data && response.data.length > 0) {
          setAppointmentFiles(response.data);
        ...
        if (defaultAppointmentFiles?.length === 0) {
    ...
    if (appointmentData.appointments_uuid) {
      fetchData();
    }
    setIsSubmitted(false);
  }, [
  ...
  const [selectedFiles, setSelectedAppointmentFiles] = useState<File[]>([]);
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      setSelectedAppointmentFiles(newFiles);
    ...
    try {
      ...
      if (selectedFiles && selectedFiles.length > 0) {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
          const appointmentFileFormData = new FormData();
          appointmentFileFormData.append(
            "appointmentfile",
            selectedFiles[i],
            fileNames[i],
          );
  ...
  return (
    <div>
      {defaultAppointmentFiles?.length === 0 && isLoading === false ? (
        <NofileviewAppointmentsModalContent
                        ...
                        <div className="flex w-full justify-between">
                          <div className="flex flex-col">
                            ...
                            <div className="mt-2 flex justify-between text-[15px] font-semibold">
                              ...
                              <div>
                                {fileIndex < AppointmentFiles.length - 1 && (
                                  <button
                                    type="button"
                                    onClick={nextFile}
                          ...
                          <div className="filehover mr-2">
                            <FileUploadWithHover />
                            {defaultAppointmentFiles.map(
                              (file: AppointmentFile, index) => (
                                <div
                                  className={` ${
                                    fileIndex === index
                                      ? "border-[#64748B]"
